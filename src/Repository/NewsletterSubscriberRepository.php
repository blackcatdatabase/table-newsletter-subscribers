...
use BlackCat\Database\Packages\NewsletterSubscribers\Repository\NewsletterSubscriberRepositoryInterface;
...
class NewsletterSubscriberRepository implements NewsletterSubscriberRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'tenant_id', 'email_hash' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\NewsletterSubscribers\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'user_id', 'email_hash_key_version', 'email_enc', 'email_key_version', 'confirm_selector', 'confirm_validator_hash', 'confirm_key_version', 'confirm_expires', 'confirmed_at', 'unsubscribe_token_hash', 'unsubscribe_token_key_version', 'unsubscribed_at', 'origin', 'ip_hash', 'ip_hash_key_version', 'meta', 'updated_at', 'version' ]);
          return count($rows);
        }
    ...
    public function getByTenantIdAndEmailHash(int $tenantId, string $emailHash, bool $asDto = false): array|\BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'email_hash' => $emailHash ], $asDto);
    }
    ...
    public function getIdByTenantIdAndEmailHash(int $tenantId, string $emailHash) {
        $row = $this->getByTenantIdAndEmailHash($tenantId, $emailHash, false);
    ...
    public function getByConfirmSelector(string $confirmSelector, bool $asDto = false): array|\BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto|null {
        return $this->getByUnique([ 'confirm_selector' => $confirmSelector ], $asDto);
    }
    ...
    public function getIdByConfirmSelector(string $confirmSelector) {
        $row = $this->getByConfirmSelector($confirmSelector, false);
        if (!is_array($row)) { return null; }
        return $row['id'] ?? null;
    }
