...
namespace BlackCat\Database\Packages\NewsletterSubscribers\Repository;
...
use BlackCat\Database\Packages\NewsletterSubscribers\Criteria;
use BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto as Dto;
use BlackCat\Database\Packages\NewsletterSubscribers\Mapper\NewsletterSubscriberDtoMapper as RowMapper;
...
final class NewsletterSubscriberRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\NewsletterSubscribers\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $keys = [ 'tenant_id', 'email_hash' ];
        if (!$keys) {
            $uqs = Definitions::uniqueKeys();
            $keys = (is_array($uqs) && isset($uqs[0]) && is_array($uqs[0]) && $uqs[0]) ? $uqs[0] : $this->pkColumns(Definitions::class);
        }

        $updCols = [ 'user_id', 'email_hash_key_version', 'email_enc', 'email_key_version', 'confirm_selector', 'confirm_validator_hash', 'confirm_key_version', 'confirm_expires', 'confirmed_at', 'unsubscribe_token_hash', 'unsubscribe_token_key_version', 'unsubscribed_at', 'origin', 'ip_hash', 'ip_hash_key_version', 'meta', 'updated_at', 'version' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'tenant_id', 'email_hash' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\NewsletterSubscribers\Definitions::table());
            return $bulk->upsertMany($rows, [ 'tenant_id', 'email_hash' ], [ 'user_id', 'email_hash_key_version', 'email_enc', 'email_key_version', 'confirm_selector', 'confirm_validator_hash', 'confirm_key_version', 'confirm_expires', 'confirmed_at', 'unsubscribe_token_hash', 'unsubscribe_token_key_version', 'unsubscribed_at', 'origin', 'ip_hash', 'ip_hash_key_version', 'meta', 'updated_at', 'version' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto|null */
    public function getByTenantIdAndEmailHash(int $tenantId, string $emailHash, bool $asDto = false): array|\BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'email_hash' => $emailHash ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\NewsletterSubscribers\Mapper\NewsletterSubscriberDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndEmailHash(int $tenantId, string $emailHash): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'email_hash') . ' = :uniq_email_hash';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_email_hash' => $emailHash ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndEmailHash(int $tenantId, string $emailHash) {
        $row = $this->getByTenantIdAndEmailHash($tenantId, $emailHash);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto|null */
    public function getByConfirmSelector(string $confirmSelector, bool $asDto = false): array|\BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto|null {
        $row = $this->getByUnique([ 'confirm_selector' => $confirmSelector ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\NewsletterSubscribers\Mapper\NewsletterSubscriberDtoMapper::fromRow($row);
    }
    public function existsByConfirmSelector(string $confirmSelector): bool {
        $where = 't.' . Ident::q($this->db, 'confirm_selector') . ' = :uniq_confirm_selector';
        return $this->exists($where, [ 'uniq_confirm_selector' => $confirmSelector ]);
    }
    /** @return int|string|null */
    public function getIdByConfirmSelector(string $confirmSelector) {
        $row = $this->getByConfirmSelector($confirmSelector);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\NewsletterSubscribers\Dto\NewsletterSubscriberDto|null {
